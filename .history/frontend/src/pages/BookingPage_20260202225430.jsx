...
const BookingPage = () => {
    ...
    const fetchShowtime = async () => {
        try {
            ...
            if (res.data.movie?._id && res.data.theater?._id) {
                const allRes = await API.get(`/showtimes/movie/${res.data.movie._id}`);
                // Filter to same theater and date - only include valid showtimes with theater and _id
    ...
    return (
        <div className="booking-page-bms">
            ...
            <div className="showtime-selector">
                {allShowtimes.filter(st => st && st._id).map(st => (
                    <button
                        key={st._id}
