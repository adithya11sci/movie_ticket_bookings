...
const MyBookings = () => {
    ...
    // Filter out bookings with null showTime first, then apply filter
    const validBookings = bookings.filter(booking => booking && booking.showTime);
    ...
    // Include cancelled bookings even if showTime is null
    const cancelledWithNullShowtime = bookings.filter(b => b && !b.showTime && b.bookingStatus === 'cancelled');
    if (filter === 'all' || filter === 'cancelled') {
        filteredBookings.push(...cancelledWithNullShowtime);
    }

    const getBookingStats = () => {
        const validBookingsForStats = bookings.filter(b => b && b.showTime);
        const upcoming = validBookingsForStats.filter(b => isUpcoming(b.showTime?.showDate) && b.bookingStatus !== 'cancelled').length;
        const past = validBookingsForStats.filter(b => !isUpcoming(b.showTime?.showDate) && b.bookingStatus !== 'cancelled').length;
        const cancelled = bookings.filter(b => b && b.bookingStatus === 'cancelled').length;
